...
import toast from "react-hot-toast";
 ...
 const UserDetails = (props: {params: { id:string }}) => {
  ...
  const {data: userData, isLoading: loadingUserData, error: errorGettingUserData} = useSWR("/api/users", fetchUserData);
    ...
    if(loadingUserData) return <LoadingSpiner/>;
    if(!userData) throw new Error("Can't fetch data");
      ...
      <div className="grid md:grid-cols-12 gap-10">
        <div className="hidden md:block md:col-span-4 lg:col-span-3 shadow-lg h-fit sticky top-10 bg-[#eff0f2] text-black rounded-lg px-6 py-4">
            <div className="md:w-[143px] w-38 h-28 md:h-[143px] mx-auto mb-5 rounded-full overflow-hidden">
                ...
                className="imd scale-animation rounded-full " />
            </div>
        ...
        <div className="md:col-span-8 lg:col-span-9">
            ...
            <div className="md:hidden w-14 h-14 rounded-l-full overflow-hidden">
                <Image className="img scale-animation rounded-full" width={56} height={56} 
                src={userData.image} alt="User Name"/>
            </div>
