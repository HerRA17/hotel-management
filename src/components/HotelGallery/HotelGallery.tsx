...
import { ImageRoom } from "@/app/models/room";
import { FaArrowLeft, FaArrowRight } from "react-icons/fa";
import { MdCancel } from "react-icons/md";

const HotelGallery: FC<{ photos: ImageRoom[] }> = ({photos}) => {
    ...
    return <div className="container mx-auto">
            <div className="grid md:grid-cols-2 relative gap-5 px-3">
                ...
                <div className="hidden md:grid grid-cols-2 h-full items-center gap-5">
                    {displayPhotos.map((photo, index) => ( 
                        <div 
                        key={index}
                        className="cursor-pointer h-64 rounded-2xl overflow-hidden">
                            <Image width={150} height={150} src={photo.url} alt={`Room Photo ${index + 2}`} 
